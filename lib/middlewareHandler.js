...
/**
 * Runs the middleware in background.
 * @param {Function}                middleware          Middleware to run in background.
 * @returns {Function}                                  A middleware.
 */
let runInBackground = function(middleware){
    //Return a middleware.
    return function(req, res, next){
        //Pass along.
        next();

        //Call the wrapped middleware, res is an empty object and next is an empty function.
    ...
    for (let i = 0 ; i < args.length ; i++){
        ...
        //Otherwise, try to get the parameter value.
            ...
            for(let source of sources){
                if(source === args[i]) value = req[args[i]];
                //Get the value from the source.
                else value = lodash.get(req, source + "." + args[i]);
                //If found, break.
                if(value) break;
            }
 ...
 * @returns {Function}          Function used by the programmer in service functions to send responses.
 */
function emptyResultHandler(_, res, next, prefs){
...
module.exports = {
    ...
    resultHandlers      : {
        ...
        emptyResult : emptyResultHandler
    },
